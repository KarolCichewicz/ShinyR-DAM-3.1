# This function was adapted from the xps package. 
#I added parameters allowing to change the window of period testing, and adjusts the resolution
...
# This function comes from the xsp package.    
...
oscillation.df <- reactive({
  ...
  data.frame(dateTime = rep(1:1440, length(unique(s_10()$date))), 
             value = rep(s_12()[,1], each = as.numeric(input$data_recording_frequency)))
...
# Melts    
df_m_Qp.act <- reactive(reshape2::melt(df_Qp.act()))
df_m_Qp.sig <- reactive(reshape2::melt(df_Qp.sig()))
...
df_period <- reactive({
  ...
  #data.frame(
  #  Condition = rep(cond, each=nrow(dQam)/length(cond)), # 5856
  #  channel = dQam$Var2,
  #  Period = rep(kk, nrow(dQam)/length(kk)),
  #  Qp.act = dQam$value,
  #  Qp.sig = dQsm$value,
  #  Act_Sig_ratio = dQam$value/dQsm$value
  #)
