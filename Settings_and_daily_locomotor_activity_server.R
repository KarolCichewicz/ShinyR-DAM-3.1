...
Data_recording_status_m <- reactive({
  melt(data.table(Data_recording_status()), id.vars=c("date", "time"))
...
### Yes, I know it's a litttle messy, but it works!  
  ...
  if(all(p$value == 1)){ 
    ...
    print(unique(filter(reshape2::melt(m), value != 1))$variable)
...
Data_range_correctness <- reactive({
  ...
  if(nrow(na.omit(s_1())) == data_freq()*number_of_days()){ 
    ...
    d <- reshape2::melt(colSums(!is.na(s_1())))
...
melted <- reactive({
  d <- s_8()
  d <- melt(data.table(d), id.vars=c("day", "month", "year", "time", "date", "Dec_time", "ZT_time", "Dec_ZT_time", "Light_cycle"))
  d$date <- as.Date(d$date)
...
melted_by_day <- reactive({
  ...
  d <- activity_by_day_1()
  p <- reshape2::melt(d, id.vars=c("date", "Light_cycle"))
