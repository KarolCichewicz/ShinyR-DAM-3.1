...
sleep_df <- reactive({
  withProgress(message = 'Calculating sleep 1/3', min=0, max=0.6, value = 0.5, {
    ...
    roll_sleep <- function(x){rollapply(select(s_9(), x), width = 5, by = 1, FUN = sum, align = "left")}
    
    activity_sleep <- do.call("cbind", sapply(all_channels(), FUN = roll_sleep, simplify = FALSE))
    
    melted_sleep <- reshape2::melt(activity_sleep) # check how columns differ in reshape2 vs data.table, adjust column selection for d
    ...
    d <- data.frame(s_9()[1:(nrow(s_9())-4),  
                          c(1:4,as.numeric(length(colnames(s_9())) - 2), 
                            as.numeric(length(colnames(s_9())) - 1), 
                            as.numeric(length(colnames(s_9())) - 3), 
                            as.numeric(length(colnames(s_9())) - 4), 
                            as.numeric(length(colnames(s_9()))))], 
                    melted_sleep[,c(2:3)], row.names=NULL)
...
mean_of_sleep_per_condition_by_day <- reactive({
  withProgress(message = 'Calculating sleep 3/3', min=0, max=0.6, value = 0.5, {
    ...
    d <- DT[,list(mean_of_sleep_counts=as.numeric(mean(sleep_counts)), 
                  sem=as.numeric(sd(sleep_counts)/sqrt(length(sleep_counts)))), 
...
output$sleep_profile_by_day <- renderPlot({
  ...
  isolate({
    ...
    binned_sleep <- as.data.frame(binned_sleep)  # New addition
...
output$Total_sleep <- renderPlot({
  ...
  isolate({
    ...
    #s_per_c <<- sleep_per_c()
    #u_conditions <<- unique_conditions()
    #P_colors <<- Plot_colors()
    ...
    barplot_sleep_total<- ggplot(sleep_per_c(), aes(x=Condition, y=mean_of_sleep_per_condition, fill=Condition, width=.5)) + 
      ...
      coord_cartesian(ylim=c(min(sleep_per_c()$mean_of_sleep_per_condition-sleep_per_c()$sem), 
                             max(sleep_per_c()$mean_of_sleep_per_condition+sleep_per_c()$sem)))
...
output$Daytime_sleep <- renderPlot({
  ...
  isolate({
    barplot_sleep_Day<- ggplot(sleep_per_condition_Day(), aes(x=Condition, y=mean_of_sleep_per_condition, fill=Condition, width=.5)) + 
      ...
      theme(axis.title.y = element_text(size=14))+
      scale_fill_manual(values=Plot_colors())+
      coord_cartesian(ylim=c(min(sleep_per_condition_Day()$mean_of_sleep_per_condition - sleep_per_condition_Day()$sem), 
                             max(sleep_per_condition_Day()$mean_of_sleep_per_condition + sleep_per_condition_Day()$sem)))
...
output$Nighttime_sleep <- renderPlot({
  ...
  isolate({
    ...
    barplot_sleep_Night<- ggplot(sleep_per_condition_Night(), aes(x=Condition, y=mean_of_sleep_per_condition, fill=Condition, width=.5)) + 
      ...
      theme(axis.title.y = element_text(size=14))+
      scale_fill_manual(values=Plot_colors())+
      coord_cartesian(ylim=c(min(sleep_per_condition_Night()$mean_of_sleep_per_condition-sleep_per_condition_Night()$sem), 
                             max(sleep_per_condition_Night()$mean_of_sleep_per_condition+sleep_per_condition_Night()$sem)))
...
### Sleep Bout Length plot  
 ...
 ### Sleep Bout Length table        
  ...
  mmm <- reshape2::melt(mm, id.vars = c("Condition", "Light_cycle"))
